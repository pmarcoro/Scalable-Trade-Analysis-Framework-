...
1. Download **yearly data** for the selected countries of analysis / reporters (for example, the top *N* exporters of the sector in a given period*), selecting **“World”** in the *Partner* field,  **“Exports”** in the *Flow* field and the year(s) of reference desired. Due to tool limitations regarding the amount of HS commodity groups selected, it may be necessary to download multiple files. Locate the JSON files in the data\raw\export structure folder and rename them including the word "detailed" for them to be detected by the Python script.
